"""Translate LeAgent's transitional provider parser output into Pi stream events."""
...
async def canonicalize_provider_stream(
    ...
    *,
    ...
    """
    # Provider 适配器只负责产生细粒度的 ProviderEvent；这里集中维护正在构建的
    # AssistantMessage，并把它转换成上层可消费的 start/delta/done 事件。
    ...
    async for event in source:
        ...
        if isinstance(event, ProviderTextDeltaEvent):
            ...
            # 流中的 partial 决定内容块顺序，parser 的终态消息只补充 usage 等响应元数据。
