"""HTTP client helpers shared by LeAgent network integrations."""
...
def normalize_proxy_url(proxy_url: str) -> str:
    ...
    request. Treat the generic form as SOCKS5 so LeAgent can honor these proxy
...
def create_async_client(**kwargs: Any) -> httpx.AsyncClient:
    """Create an ``httpx.AsyncClient`` with LeAgent's proxy normalization applied."""
...
def get_json(url: str, *, timeout: float, follow_redirects: bool = False) -> dict[str, object]:
    """Fetch a JSON object with LeAgent's proxy normalization applied."""
