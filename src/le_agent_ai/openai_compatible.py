...
from le_agent.tools import AgentTool, ToolCall
from le_agent.types import JSONValue
from le_agent_ai._provider_events import (
    ProviderErrorEvent,
    ProviderEvent,
...
from le_agent_ai.content import (
    NON_VISION_TOOL_IMAGE_PLACEHOLDER,
    NON_VISION_USER_IMAGE_PLACEHOLDER,
    messages_have_images,
    text_and_images,
)
from le_agent_ai.env import OpenAICompatibleConfig
from le_agent_ai.events import AssistantMessageEvent
from le_agent_ai.http import create_async_client
from le_agent_ai.http_errors import provider_http_error_message
from le_agent_ai.provider import CancellationToken
from le_agent_ai.retry import provider_retry_event, retry_delay_seconds, wait_for_retry
from le_agent_ai.stream import canonicalize_provider_stream
...
class OpenAICompatibleProvider:
    ...
    def stream_response(
        ...
        *,
        ...
        # HTTP/SSE 解析先收敛为私有 ProviderEvent，再由统一桥接层组装最终消息；
        # 因而上层永远看不到 Chat Completions 与 Responses API 的 wire 差异。
    ...
    def _stream_provider_events(
        ...
        *,
        ...
        # 两种 OpenAI API 只在 payload 和 parser 上分叉，输出在此边界重新汇合。
...
def _parse_chunk_usage(raw: Mapping[str, Any]) -> Usage:
    ...
    unset (None) because LeAgent has no per-model pricing table.
    """
...
def _usage_from_responses_event(chunk: Mapping[str, Any]) -> Usage | None:
    ...
    cost is left unset because LeAgent has no per-model pricing table.
    """
    response = chunk.get("response")
