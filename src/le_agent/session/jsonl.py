"""JSONL serialization and LeAgent-v1 persisted-session migration."""
...
def entry_from_json_line(line: str, *, line_number: int | None = None) -> SessionEntry:
    """Deserialize one entry, migrating persisted LeAgent-v1 messages first."""
    location = f" on line {line_number}" if line_number is not None else ""
    try:
        payload = json.loads(line)
        # 旧格式只在反序列化入口迁移；离开此边界后，运行时始终只有一套严格协议。
