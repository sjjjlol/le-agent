"""Built-in filesystem and shell tools for LeAgent coding sessions.

本模块实现 LeAgent Coding Agent 的内置工具——文件系统操作和 Shell 命令。

架构定位：
- 位于 le_agent_coding 层，是 Coding 应用的具体工具实现
- 操作相对于可配置的工作目录（cwd）
- 返回结构化的 AgentToolResult
- 将本地文件系统/Shell 行为与可复用的 le_agent 包分离

核心设计：
1. ToolDefinition：Coding 层的完整工具定义（比 AgentTool 更丰富）
2. to_agent_tool()：将 ToolDefinition 转换为 AgentTool（适配器模式）
3. 截断策略：工具输出有字节数和行数限制，防止过大响应
4. 取消支持：长运行工具（如 shell 命令）支持取消信号
...
安全设计：
- 路径验证：防止越界访问（如读取 /etc/passwd）
- 输出限制：防止超大输出拖垮系统
- 取消机制：用户可以中断长时间运行的命令

截断策略：
- DEFAULT_MAX_OUTPUT_BYTES = 50KB（单文件最大输出）
- DEFAULT_MAX_OUTPUT_LINES = 2000 行（单行数限制）
- 超出限制时返回截断标记，告知用户结果不完整
"""
...
from le_agent.messages import ImageContent, TextContent
...
if TYPE_CHECKING:
    from le_agent_coding.web_search import WebSearchBackend
from le_agent_coding.image_processing import (
...
class ToolDefinition:
    """Coding 工具的完整定义（转换为 AgentTool 之前的形态）。

    与 AgentTool 的区别：
    - ToolDefinition 是 Coding 层的构建形态，包含更多元数据
    - AgentTool 是核心层消耗的精简形态
    - to_agent_tool() 执行转换，相当于适配器边界

    字段说明：
    - name: 工具唯一标识（如 "read", "write", "bash"）
    - description: 工具功能描述（给模型看的自然语言描述）
    - prompt_snippet: 注入系统提示词的代码片段（展示工具用法示例）
    - prompt_guidelines: 注入系统提示词的使用指南（最佳实践）
    - input_schema: JSON Schema 对象，描述参数结构
    - executor: 异步执行函数，接收参数和取消信号，返回 AgentToolResult

    to_agent_tool() 方法：
    将 ToolDefinition 转换为 AgentTool，供核心循环使用。
    转换过程：
    1. 包装 executor 为 ToolExecutor 签名
    2. 复制 name、description、parameters
    3. 保留 prompt_snippet、prompt_guidelines 供系统提示词组装

    为什么分层：
    - 应用层可以携带 cwd、校验、展示策略等丰富信息
    - 适配后核心循环只看到标准 schema 和 executor
    - 保持核心层与具体工具实现解耦
    """
    ...
    def to_agent_tool(self) -> AgentTool:
        """将 Coding 层定义转换为 Pi 兼容的核心工具。"""

        # 应用层定义携带 cwd、校验和展示策略；适配后核心循环只看到 schema 与 executor。
        async def execute(
            tool_call_id: str,
...
def create_coding_tools(
    *,
    ...
    The returned tools are ordered as `read`, `write`, `edit`, `bash`, and optionally
    `web_search`.
    ...
    """
    # 在工厂调用时冻结 cwd，避免 Agent 运行期间进程目录变化而改变相对路径含义。
    ...
    if web_search_enabled:
        from le_agent_coding.web_search import create_web_search_tool_definition

        tools.append(create_web_search_tool_definition(web_search_backend).to_agent_tool())
    return tools
