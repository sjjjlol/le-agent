# LeAgent providers and models

LeAgent separates provider/model streaming (`le_agent_ai`), the portable harness (`le_agent`), and application configuration (`le_agent_coding`).

## User configuration

Use `/login` and `/model` for built-in providers. The custom-provider flow supports OpenAI-compatible endpoints. Durable provider settings live under LeAgent's home directory; consult the published `website/content/guides/providers-and-models.md` in a LeAgent checkout for the current schema and authentication behavior.
...
3. Confirm LeAgent supports the API transport before adding a provider.
4. Update the catalog source of truth:

   ```text
   src/le_agent_coding/data/catalog.toml
   ```
...
LeAgent thinking levels are `off`, `minimal`, `low`, `medium`, `high`, and `xhigh`. Provider-level `thinking_levels` must include every level needed by its models. Use model metadata when the wire value differs or a model supports only a subset:
