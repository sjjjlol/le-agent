...
def load_skills(paths: LeAgentResourcePaths | None = None) -> list[Skill]:
    """Load markdown skills from LeAgent and `.agents` resource directories.
...
def parse_skill_invocation(text: str) -> SkillInvocation | None:
    """Parse LeAgent's expanded skill invocation message format."""
    match = re.match(
...
def _load_skills_from_dir_with_diagnostics(
    ...
    # LeAgent follows the Agent Skills spec everywhere: a skill is a directory
