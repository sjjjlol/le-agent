"""System prompt assembly for LeAgent coding sessions."""
...
from le_agent_coding.self_docs import (
    le_agent_docs_path,
    le_agent_examples_path,
    le_agent_readme_path,
)
...
class BuildSystemPromptOptions:
    """Options used to build LeAgent's system prompt."""
...
def build_system_prompt(options: BuildSystemPromptOptions) -> str:
    """Build a deterministic Pi-style system prompt for LeAgent."""
    # 各来源按固定顺序拼装，使相同 tools、skills 与项目上下文得到稳定提示词，
    # 也让调用方只负责提供资源，不必了解最终文本布局。
    ...
    prompt = (
        "You are an expert coding assistant operating inside LeAgent, a coding agent harness. "
...
def format_le_agent_documentation() -> str:
    """Format Pi-style routing hints to LeAgent's installed reference material."""
    readme_path = _format_path(le_agent_readme_path())
    docs_path = _format_path(le_agent_docs_path())
    examples_path = _format_path(le_agent_examples_path())
    return (
        "LeAgent documentation (read only when the user asks about LeAgent itself, its SDK, "
        ...
        "- When reading LeAgent docs or examples, resolve docs/... under Additional docs and "
        ...
        "(docs/tui.md), LeAgent architecture and packages (docs/architecture.md)\n"
        "- When working on LeAgent topics, read the docs and examples, and follow .md "
        "cross-references before implementing\n"
        "- Always read relevant LeAgent .md files completely and follow links to related docs"
    )
