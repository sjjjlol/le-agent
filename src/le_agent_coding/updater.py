"""Upgrade LeAgent with the package manager that owns its environment."""
...
from le_agent_coding.update_check import PYPI_PACKAGE_NAME, fetch_latest_pypi_version
...
class UpdateResult:
    """Result of trying to upgrade LeAgent."""
...
def update_le_agent(
    *,
    ...
    """Upgrade LeAgent with the installer that owns the active environment.
    ...
    if direct_url:
        return _failure(
            "LeAgent was installed from a local or direct URL, so it cannot be safely "
    ...
    if method is None:
        if (prefix / "conda-meta").is_dir():
            return _failure(
                "LeAgent is installed in a Conda/Pixi-managed environment. "
                "Update it with the manager that created that environment."
            )
        installed_by = installer or "unknown"
        return _failure(
            "Could not identify a supported installer for this LeAgent environment "
    ...
    if method == "uv-tool":
        try:
            ...
            return _failure(f"Could not determine the latest LeAgent version from PyPI: {exc}")
        if latest_version is None:
            return _failure("Could not determine the latest LeAgent version from PyPI.")
...
try {
    ...
    $WaitMessage = "Waiting for LeAgent process $ParentProcessId to exit..."
    ...
    try {
        ...
        if ($_.FullyQualifiedErrorId -like "NoProcessFoundForGivenId*") {
            Write-Log "LeAgent process $ParentProcessId has already exited."
        } else {
            $Detail = $_.Exception.Message
            throw "Could not inspect LeAgent process $ParentProcessId. Update aborted: $Detail"
        }
    }
    if ($null -ne $ParentProcess) {
        try {
            ...
            $Detail = $_.Exception.Message
            throw "Could not wait for LeAgent process $ParentProcessId to exit. " +
...
def _handoff_windows_update(
    ...
    *,
    ...
    try:
        if owns_update_dir:
            update_dir = Path(tempfile.mkdtemp(prefix="le-agent-update-"))
        else:
    ...
    return UpdateResult(
        command=command,
        stdout=(
            "LeAgent update is scheduled and will start after this process exits. "
