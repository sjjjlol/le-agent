...
from le_agent_coding.resources import LeAgentResourcePaths, ResourceDiagnostic

EXTENSION_ENTRY_ATTRIBUTE = "setup"
_MODULE_NAME_PREFIX = "le_agent_extension"
...
def extension_dirs(
    ...
    *,
    ...
    Project extensions are opt-in (`--project-extensions`) until LeAgent has a
    project trust store, because they execute at session startup.
    """
    dirs: list[Path] = []
    if include_project_dir and paths.cwd is not None:
        dirs.append(paths.cwd / ".le-agent" / "extensions")
...
def _manifest_entries(
    ...
    """Resolve entry files declared in `<dir>/pyproject.toml` under `[tool.le-agent]`.
    ...
    use package.json manifest"); `tool.le-agent.extensions` is the Python-shaped
    ...
    le_agent_table = tool_table.get("le-agent") if isinstance(tool_table, dict) else None
    declared = le_agent_table.get("extensions") if isinstance(le_agent_table, dict) else None
    if declared is None:
        return []
    if not isinstance(declared, list) or not all(isinstance(item, str) for item in declared):
        diagnostics.append(
            ResourceDiagnostic(
                kind="extension",
                path=manifest,
                message="`tool.le-agent.extensions` must be a list of file paths",
                severity="error",
            )
        )
        return []
    ...
    if not callable(setup):
        ...
        return None, [
            _error_diagnostic(
                ...
                f"`{EXTENSION_ENTRY_ATTRIBUTE}(le_agent)` entry point",
            )
        ]
