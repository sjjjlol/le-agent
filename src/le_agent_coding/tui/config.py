"""Durable Textual TUI configuration for LeAgent."""
...
class TuiConfigError(ValueError):
    """Raised when LeAgent TUI configuration is invalid."""


@dataclass(frozen=True, slots=True)
class TuiKeybindings:
    """Configurable keys for LeAgent's built-in Textual frontend."""
...
@dataclass(frozen=True, slots=True)
class TuiSettings:
    """LeAgent TUI settings loaded from LeAgent home."""

    keybindings: TuiKeybindings = field(default_factory=TuiKeybindings)
    theme: TuiThemeName = "le-agent-dark"
    ...
    def resolved_theme(self) -> TuiTheme:
        """Return the selected theme, falling back to le-agent-dark when unknown."""
        try:
...
def tui_settings_path(paths: LeAgentPaths | None = None) -> Path:
    """Return the durable TUI settings path."""
    return (paths or LeAgentPaths()).home / "tui.json"


def load_tui_settings(paths: LeAgentPaths | None = None) -> TuiSettings:
...
def save_tui_settings(settings: TuiSettings, paths: LeAgentPaths | None = None) -> Path:
...
def tui_settings_from_json(data: dict[str, Any]) -> TuiSettings:
    ...
    # Ignore settings added by newer LeAgent versions so sharing this user-level
    ...
    return TuiSettings(
        ...
        theme=_theme_name(data.get("theme", "le-agent-dark")),
