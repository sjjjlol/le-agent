...
from le_agent.messages import AssistantMessage, CustomMessage, ToolCall, UserMessage
from le_agent_ai.events import TextDeltaEvent, ThinkingDeltaEvent
...
from le_agent_coding.session import is_context_overflow_error
from le_agent_coding.tui.state import TuiState
...
class TuiEventAdapter:
    ...
    def apply(self, event: CodingSessionEvent) -> None:
        # 这是从事件到 UI state 的单向投影；adapter 不回调 Harness，也不拥有 transcript。
        ...
        if isinstance(event, MessageEndEvent):
            ...
            if isinstance(message, UserMessage):
                ...
                # delta 只负责即时显示，最终消息才决定持久化后的内容块顺序与终止状态。
