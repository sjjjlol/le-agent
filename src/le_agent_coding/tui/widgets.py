"""Small Textual widgets for LeAgent's interactive TUI."""
...
from le_agent_coding.tui.autocomplete import CompletionState
from le_agent_coding.tui.config import LE_AGENT_DARK_THEME, TuiRoleStyle, TuiTheme
from le_agent_coding.tui.state import ChatItem, TuiState
from le_agent_coding.version import current_version

LE_AGENT_SIDEBAR_LOGO = "LeAgent"
...
class SessionSidebar(Vertical):
    ...
    def update_from_session(
        ...
        *,
        theme: TuiTheme = LE_AGENT_DARK_THEME,
    ) -> None:
...
class LeAgentMarkdownBlock(MarkdownBlock):
    """Markdown block that applies LeAgent's themed inline link color."""
    ...
    def _token_to_content(self, token: Any) -> Any:
        ...
        if not isinstance(markdown, ThemedMarkdownWidget):
            return content
        link_style = TextualStyle.parse(markdown.le_agent_link_style)
        spans = []
...
class ThemedMarkdownWidget(TextualMarkdown):
    """Textual Markdown widget reserved for LeAgent transcript streaming."""

    BLOCKS = {**TextualMarkdown.BLOCKS, "paragraph_open": LeAgentMarkdownBlock}
    ...
    ThemedMarkdownWidget MarkdownH6 {
        color: $le-agent-markdown-highlight;
        content-align: left middle;
        text-style: bold;
    }

    ThemedMarkdownWidget MarkdownBlock > .code_inline {
        color: $le-agent-markdown-inline-code !important;
        background: transparent !important;
    }

    ThemedMarkdownWidget MarkdownBullet {
        color: $le-agent-markdown-bullet;
    }

    ThemedMarkdownWidget MarkdownFence {
        background: $le-agent-markdown-code-block-background;
        overflow-x: auto;
    ...
    ThemedMarkdownWidget MarkdownTableContent {
        keyline: thin $le-agent-markdown-table-border;
    }

    ThemedMarkdownWidget MarkdownTableContent > .header {
        color: $le-agent-markdown-table-header;
        text-style: bold;
    }
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.le_agent_link_style = theme.markdown_link
...
class TranscriptView(VerticalScroll):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self._render_theme: TuiTheme = LE_AGENT_DARK_THEME
...
def _sidebar_brand(*, theme: TuiTheme) -> RenderableType:
    ...
    brand.append(LE_AGENT_SIDEBAR_LOGO)
...
class ThemedCodeBlock(CodeBlock):
    """Rich Markdown code block with LeAgent's themed background color."""
...
class ThemedMarkdown(Markdown):
    """Markdown renderer with LeAgent's softer heading/accent colors."""
