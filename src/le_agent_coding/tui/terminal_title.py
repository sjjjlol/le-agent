"""Helpers for updating the terminal window/tab title from LeAgent's TUI."""
...
def terminal_title_supported(
    *,
    ...
    """Return whether LeAgent should emit OSC title sequences in this process."""
    env = os.environ if environ is None else environ
    if env.get("LE_AGENT_TERMINAL_TITLE", "").lower() in {"0", "false", "no", "off"}:
        return False
    ...
    if env.get("TERM", "") == "dumb":
        return False
    return not (env.get("CI", "") and env.get("LE_AGENT_TERMINAL_TITLE", "").lower() != "1")
...
def build_terminal_title(
    ...
    *,
    ...
    """Return LeAgent's terminal tab title for the current session/running state."""
    title = sanitize_terminal_title(session_title)
    title = (
        LE_AGENT_TITLE_MARK
        if not title or title.lower() == "untitled session"
        else f"{LE_AGENT_TITLE_MARK} | {title}"
    )
    if not running:
        return title
...
class TerminalTitleController:
    ...
    def __init__(
        ...
        *,
        ...
        stream: TextIO | None = None,
        environ: Mapping[str, str] | None = None,
        exit_title: str = LE_AGENT_TITLE_MARK,
    ) -> None:
    ...
    def update(self, session_title: str | None, *, running: bool, frame: int = 0) -> None:
        """Write the current LeAgent title if it differs from the last emitted title."""
        if not self.enabled:
            return
    ...
    def restore(self) -> None:
        """Leave the terminal title in a neutral idle LeAgent state on shutdown."""
        if not self.enabled:
            return
