"""Minimal Textual app for LeAgent coding sessions."""
...
from le_agent.provider import CancellationToken
...
from le_agent_coding.catalog_loader import save_user_catalog_entries
from le_agent_coding.commands import (
...
from le_agent_coding.oauth import login_openai_codex
from le_agent_coding.oauth_registry import get_oauth_provider, oauth_provider_ids
from le_agent_coding.oauth_types import (
    OAuthAuthInfo,
    OAuthDeviceCodeInfo,
    OAuthLoginCallbacks,
    OAuthPrompt,
...
from le_agent_coding.provider_catalog import (
...
from le_agent_coding.session_manager import CodingSessionRecord, SessionManager
from le_agent_coding.shell_config import load_shell_settings
from le_agent_coding.skills import Skill
from le_agent_coding.tui.adapter import TuiEventAdapter
from le_agent_coding.tui.autocomplete import (
    CompletionItem,
    CompletionOption,
    CompletionState,
...
from le_agent_coding.tui.file_drop import normalize_dropped_paths
from le_agent_coding.tui.state import TuiState, format_terminal_command_result_block
...
PROMPT_PLACEHOLDER = "Ask LeAgent…  Enter submits, Shift+Enter inserts a newline"
...
class _TuiExtensionUiBridge:
    ...
    def __init__(self, app: LeAgentTuiApp) -> None:
        self._app = app
    ...
    def register_key_interceptor(self, handler: KeyInterceptor) -> Callable[[], None]:
        ...
        ``LeAgentTuiApp.on_event`` before Textual's app-level priority bindings and
        before the focused widget receives the key, so it can own navigation
        keys (``up``/``down``/``tab``/…) that le-agent otherwise binds with
...
class _MainViewHandle:
    ...
    def __init__(self, app: LeAgentTuiApp, result: asyncio.Future[object | None]) -> None:
        self._app = app
        self._open = True
...
class PromptInput(TextArea):
    ...
    def handle_pasted_text(self, text: str) -> bool:
        """Apply LeAgent's paste rules to *text*.
    ...
    async def on_key(self, event: Key) -> None:
        ...
        :meth:`LeAgentTuiApp.on_event` (pre-dispatch, before app-level priority
...
#: (LeAgent binds it to ``clear_prompt``, but it is the terminal-standard
#: SIGINT/interrupt reflex users hit to bail). NOT reserved: escape/enter/
#: arrows/tab/left/right — those are load-bearing for the le-agent-subagents
#: extension and must stay interceptable. This is LeAgent's counterpart to Pi's
...
class LeAgentTuiApp(App[None]):
    ...
    Screen {
        layout: vertical;
        background: $le-agent-screen-background;
        color: $le-agent-screen-text;
    }

    Toast {
        background: $le-agent-chrome-background;
        color: $le-agent-chrome-text;
    }

    Toast .toast--title {
        color: $le-agent-accent;
    }
    ...
    #sidebar {
        ...
        height: 1fr;
        padding: 1 1 1 2;
        background: $le-agent-prompt-background;
        border: none;
    }
    ...
    #sidebar-brand {
        height: auto;
        color: $le-agent-prompt-text;
    }

    LeAgentTuiApp.-hide-sidebar #sidebar {
        display: none;
    }

    LeAgentTuiApp.-hide-sidebar #main-pane {
        padding-left: 1;
    }

    LeAgentTuiApp.-sidebar-right #sidebar {
        dock: right;
    }
    ...
    #transcript {
        height: 1fr;
        border: none;
        background: $le-agent-transcript-background;
        padding: 0 0 0 2;
        overflow-x: auto;
    ...
    #prompt:focus {
        border-left: tall $le-agent-prompt-border;
    }

    #prompt.-shell-mode {
        border-left: tall $le-agent-tool-running;
    }
    ...
    #autocomplete {
        ...
        margin: 0 1 1 1;
        padding: 0 1;
        background: $le-agent-autocomplete-background;
        color: $le-agent-screen-text;
        border: tall $le-agent-border;
        overflow-y: auto;
    }
    ...
    #tools-reference-search {
        ...
        background: $le-agent-prompt-background;
        color: $le-agent-prompt-text;
        border: tall $le-agent-prompt-border;
    }
    ...
    ListView > ListItem.-highlight {
        background: $le-agent-highlight-background;
        color: $le-agent-highlight-text;
    }
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self._register_le_agent_textual_themes()
    ...
    def _register_le_agent_textual_themes(self) -> None:
        """Register LeAgent themes with Textual's theme system.

        Textual exposes its own theme menu and command palette entries. Registering
        LeAgent's built-in themes there makes those controls update the same theme as
        `/theme` instead of changing only Textual's chrome.
        """
        self._registered_themes.clear()
        for theme_name in available_tui_theme_names():
            self.register_theme(_textual_theme_for_le_agent_theme(theme_name))

    def _watch_theme(self, theme_name: str) -> None:
        """Keep Textual theme changes synchronized with LeAgent's durable TUI theme."""
        super()._watch_theme(theme_name)
        if theme_name not in available_tui_theme_names():
            return
        if getattr(self, "_applying_settings_theme", False):
            return
        le_agent_theme: TuiThemeName = theme_name
        if self.tui_settings.theme == le_agent_theme:
            return
        self._replace_tui_settings(theme=le_agent_theme)
        save_tui_settings(self.tui_settings)

    def get_theme_variable_defaults(self) -> dict[str, str]:
        """Return LeAgent-specific CSS variables for the selected TUI theme."""
        ...
        with Horizontal(id="workspace"):
            ...
            with Vertical(id="main-pane"):
                ...
                with Horizontal(id="prompt-row"):
                    yield Static("L", id="prompt-prefix")
                    yield PromptInput(
    ...
    async def on_event(self, event: events.Event) -> None:
        ...
        the earliest point in key processing — before le-agent's app-level priority
    ...
    def on_app_focus(self) -> None:
        """Suppress turn notifications while the LeAgent terminal surface is active."""
        self._app_has_focus = True
    ...
    def action_open_session_picker(self) -> None:
        ...
        if self.state.running:
            self._notify("LeAgent is already working. Press Escape to cancel.")
            return
...
def _render_activity_indicator(
    ...
    *,
    ...
    if not running:
        return Text("L", style=f"bold {theme.accent}")
...
def _textual_theme_for_le_agent_theme(theme_name: TuiThemeName) -> Theme:
    """Map a LeAgent theme to Textual's native theme type."""
...
def _theme_css_variables(theme: TuiTheme) -> dict[str, str]:
    """Return Textual CSS variables for a resolved LeAgent theme."""
    return {
        "le-agent-screen-background": theme.screen_background,
        "le-agent-screen-text": theme.screen_text,
        "le-agent-chrome-background": theme.chrome_background,
        "le-agent-chrome-text": theme.chrome_text,
        "le-agent-muted-text": theme.muted_text,
        "le-agent-sidebar-background": theme.sidebar_background,
        "le-agent-border": theme.border,
        "le-agent-transcript-background": theme.transcript_background,
        "le-agent-prompt-background": theme.prompt_background,
        "le-agent-prompt-text": theme.prompt_text,
        "le-agent-prompt-border": theme.prompt_border,
        "le-agent-autocomplete-background": theme.autocomplete_background,
        "le-agent-accent": theme.accent,
        "le-agent-tool-running": theme.role_styles["tool"].border,
        "le-agent-highlight-background": theme.highlight_background,
        "le-agent-highlight-text": theme.highlight_text,
        "le-agent-markdown-highlight": theme.markdown_heading,
        "le-agent-markdown-table-header": theme.markdown_table_header,
        "le-agent-markdown-table-border": theme.markdown_table_border,
        "le-agent-markdown-inline-code": theme.markdown_inline_code,
        "le-agent-markdown-code-block-background": theme.markdown_code_block_background,
        "le-agent-markdown-link": theme.markdown_link,
        "le-agent-markdown-bullet": theme.markdown_bullet,
...
def _attach_retry_hint_to_error(state: TuiState, message: AssistantMessage) -> None:
    ...
    they are skipped to avoid asking the user to retry while LeAgent already is.
    """
    ...
    *,
    ...
    try:
        ...
        custom_themes, theme_diagnostics = load_custom_tui_themes(
            LeAgentResourcePaths(cwd=record.cwd).themes_dirs
        )
