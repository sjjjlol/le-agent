"""JSON-defined TUI themes for LeAgent.
...
``~/.le-agent/themes/*.json`` and ``<project>/.le-agent/themes/*.json``.
"""
...
@dataclass(frozen=True, slots=True)
class TuiTheme:
    """Resolved visual theme for LeAgent's built-in Textual frontend."""
...
LE_AGENT_DARK_THEME = _load_builtin_theme("le-agent-dark.json")
LE_AGENT_LIGHT_THEME = _load_builtin_theme("le-agent-light.json")
HIGH_CONTRAST_THEME = _load_builtin_theme("high-contrast.json")

_BUILTIN_THEMES: dict[str, TuiTheme] = {
    theme.name: theme for theme in (LE_AGENT_DARK_THEME, LE_AGENT_LIGHT_THEME, HIGH_CONTRAST_THEME)
}
...
def get_tui_theme(name: TuiThemeName = "le-agent-dark") -> TuiTheme:
