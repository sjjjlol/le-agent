"""Durable provider configuration for LeAgent coding sessions."""
...
from le_agent_ai.openai_codex import DEFAULT_OPENAI_CODEX_BASE_URL
from le_agent_coding.catalog_loader import effective_catalog, save_user_catalog_entries
from le_agent_coding.credentials import FileCredentialStore, credentials_path
from le_agent_coding.oauth_registry import get_oauth_provider
...
class ProviderConfigError(ValueError):
    """Raised when LeAgent provider configuration is invalid."""
...
class ProviderSettings:
    """LeAgent provider settings loaded from LeAgent home."""
...
class ProviderSelection:
    """Resolved provider/model selection for a LeAgent run."""
...
def builtin_provider_configs() -> tuple[ProviderConfig, ...]:
    """Return LeAgent's built-in provider configs."""
    return tuple(
...
def default_openai_provider_config() -> OpenAICompatibleProviderConfig:
    """Return LeAgent's default OpenAI-compatible provider entry."""
...
def provider_settings_path(paths: LeAgentPaths | None = None) -> Path:
    """Return the durable provider settings path."""
    return (paths or LeAgentPaths()).home / "providers.json"
...
def load_provider_settings(paths: LeAgentPaths | None = None) -> ProviderSettings:
    ...
    resolved_paths = paths or LeAgentPaths()
...
def save_provider_settings(settings: ProviderSettings, paths: LeAgentPaths | None = None) -> Path:
...
def _save_migrated_provider_settings(settings: ProviderSettings, *, paths: LeAgentPaths) -> None:
...
def _effective_provider_configs(paths: LeAgentPaths | None = None) -> tuple[ProviderConfig, ...]:
    ...
    *,
    ...
    for name, preference_data in value.items():
        ...
        if provider_name not in catalog_configs:
            ...
            # preference behind. Ignore it so one stale entry cannot prevent LeAgent
            # from starting or running `le-agent setup` to register it again.
            continue
        providers.append(
...
def _apply_provider_preference(
    ...
    # Provider preferences are user-level state shared across LeAgent versions.
    ...
    *,
    ...
    if provider.thinking_levels is None:
        ...
        if isinstance(provider, OpenAICodexProviderConfig):
            return (
                "OpenAI Codex subscription can stream reasoning output, but LeAgent does "
...
def provider_kind(provider: ProviderConfig) -> ProviderKind:
    ...
    if (
        ...
        and provider.api == "mistral-conversations"
    ):
        return "mistral-conversations"
...
def provider_has_usable_credentials(
    ...
    *,
    ...
    """Return whether LeAgent can attempt calls for this provider without prompting setup."""
...
def _reasoning_effort_from_provider(
    ...
    *,
    ...
    if provider.name == "huggingface" and normalized == "minimal":
        ...
        # for reasoning_effort, but rejects Pi/LeAgent's "minimal" label.
        return "low"
