"""Project instruction discovery for LeAgent coding sessions."""
...
def discover_project_context(
    paths: LeAgentResourcePaths | None = None,
...
def discover_project_context_with_diagnostics(
    ...
    resource_paths = paths or LeAgentResourcePaths()
...
def _context_file_candidates(paths: LeAgentResourcePaths) -> tuple[Path, ...]:
    ...
    if paths.cwd is not None:
        ...
        le_agent_paths = paths._paths()
        candidates.extend(
            [
                le_agent_paths.project_le_agent_dir(cwd) / "AGENTS.md",
                le_agent_paths.project_agents_dir(cwd) / "AGENTS.md",
            ]
        )
