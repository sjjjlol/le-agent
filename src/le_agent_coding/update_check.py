"""Best-effort PyPI update checks for the LeAgent CLI."""
...
from le_agent_ai.http import get_json
from le_agent_coding.paths import LeAgentPaths

PYPI_PACKAGE_NAME = "le-agent"
...
UPDATE_CHECK_ENV_DISABLE = "LE_AGENT_NO_UPDATE_CHECK"
...
class UpdateNotice:
    ...
    def message(self) -> str:
        ...
        return (
            f"LeAgent {self.latest_version} is available (installed: {self.current_version}). "
            "Run `le-agent update` to upgrade."
        )
...
class ReleaseNotesEntry:
    """Structured release notes for one LeAgent version."""
...
class ReleaseNotesNotice:
    """Release notes shown once after the installed LeAgent version changes."""
    ...
    def message(self) -> str:
        ...
        return f"LeAgent updated to {self.current_version}\n\n{body}"
...
def startup_update_notice(
    ...
    *,
    ...
    """Return an update notice when PyPI has a newer stable LeAgent release.
...
def startup_release_notes_notice(
    ...
    *,
    ...
    """Return release notes once after LeAgent starts with a newer installed version.
...
def fetch_latest_pypi_version(*, fetcher: Fetcher | None = None) -> str | None:
    """Fetch the latest stable LeAgent version from PyPI."""
...
def default_update_check_cache_path(paths: LeAgentPaths | None = None) -> Path:
    """Return the on-disk cache path for startup update checks."""
    return (paths or LeAgentPaths()).home / "cache" / "update-check.json"


def default_release_notes_state_path(paths: LeAgentPaths | None = None) -> Path:
    """Return the on-disk state path for one-time release notes."""
    return (paths or LeAgentPaths()).home / "cache" / RELEASE_NOTES_STATE_FILENAME
