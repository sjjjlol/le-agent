"""Load LeAgent's provider catalog from packaged and user TOML files."""
...
from le_agent_coding.thinking import ThinkingLevel, ThinkingParameter
...
_UNSUPPORTED_NATIVE_PROVIDER_KINDS = {"anthropic", "google-generative-ai"}
_UNSUPPORTED_NATIVE_PROVIDER_APIS = {"anthropic-messages", "google-generative-ai"}
...
class CatalogError(ValueError):
    """Raised when a LeAgent catalog file is invalid."""
...
def builtin_catalog_resource_text() -> str:
    ...
    return files("le_agent_coding").joinpath("data/catalog.toml").read_text(encoding="utf-8")


@cache
def builtin_catalog() -> tuple[ProviderCatalogEntry, ...]:
    """Return LeAgent's built-in provider catalog from the packaged data file."""
    raw = _builtin_raw()
    filtered = _filter_supported_providers(_apply_model_tombstones(raw, base=raw))
    return _entries_from_raw(filtered, source="built-in catalog.toml")


def user_catalog_path(paths: LeAgentPaths | None = None) -> Path:
    """Return the user-level catalog overlay path."""
    return (paths or LeAgentPaths()).home / USER_CATALOG_FILENAME


def effective_catalog(paths: LeAgentPaths | None = None) -> tuple[ProviderCatalogEntry, ...]:
    """Return the builtin catalog with the user's ~/.le-agent/catalog.toml overlaid."""
    ...
    filtered = _filter_supported_providers(_apply_model_tombstones(merged, base=builtin_raw))
...
def _filter_supported_providers(raw: dict[str, Any]) -> dict[str, Any]:
    """Drop native provider protocols that are not shipped by le-agent."""
    providers: list[dict[str, Any]] = []
    for original in _raw_providers(raw):
        kind = original.get("kind")
        api = original.get("api")
        if kind in _UNSUPPORTED_NATIVE_PROVIDER_KINDS:
            continue
        if api in _UNSUPPORTED_NATIVE_PROVIDER_APIS:
            continue
        ...
        if isinstance(metadata, dict):
            unsupported_models = {
                model
                for model, value in metadata.items()
                if isinstance(value, dict)
                and value.get("api") is not None
                and value.get("api") in _UNSUPPORTED_NATIVE_PROVIDER_APIS
            }
        if unsupported_models:
            for field in ("models", "thinking_models"):
                values = provider.get(field)
                if isinstance(values, list):
                    provider[field] = [value for value in values if value not in unsupported_models]
            for field in ("context_windows", "model_metadata"):
                values = provider.get(field)
                if isinstance(values, dict):
                    provider[field] = {
                        ...
                        if model not in unsupported_models
                    }
            if provider.get("default_model") in unsupported_models:
                remaining = provider.get("models")
                if not isinstance(remaining, list) or not remaining:
                    continue
                provider["default_model"] = remaining[0]
        providers.append(provider)
    return {**raw, "providers": providers}
