"""Canonical filesystem paths for LeAgent user and project data."""
...
class LeAgentPaths:
    """Resolved LeAgent filesystem locations.

    LeAgent keeps durable application data under the user's home directory while also
    loading project-local resources from the active working directory.
    """

    home: Path = field(default_factory=lambda: Path.home() / ".le-agent")
    ...
    def logs_dir(self) -> Path:
        """Return LeAgent's user-level diagnostic log directory."""
    ...
    def user_skills_dir(self) -> Path:
        """Return LeAgent's user-level skills directory."""
        return self.home / "skills"

    @property
    def user_prompts_dir(self) -> Path:
        """Return LeAgent's user-level prompt templates directory."""
        return self.home / "prompts"

    @property
    def user_themes_dir(self) -> Path:
        """Return LeAgent's user-level TUI themes directory."""
    ...
    def project_le_agent_dir(self, cwd: Path) -> Path:
        """Return the project-local LeAgent resource directory."""
        return cwd / ".le-agent"
    ...
    def project_skills_dir(self, cwd: Path) -> Path:
        """Return the project-local LeAgent skills directory."""
        return self.project_le_agent_dir(cwd) / "skills"

    def project_prompts_dir(self, cwd: Path) -> Path:
        """Return the project-local LeAgent prompt templates directory."""
        return self.project_le_agent_dir(cwd) / "prompts"

    def project_themes_dir(self, cwd: Path) -> Path:
        """Return the project-local LeAgent TUI themes directory."""
        return self.project_le_agent_dir(cwd) / "themes"
