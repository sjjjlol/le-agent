...
本模块实现 CodingSession 类——LeAgent Coding Agent 的核心会话管理器。

架构设计：
- CodingSession 不是第二个 Agent Loop，而是包围 Harness 的 Coding 应用层包装器
- AgentHarness 拥有内存中的 Agent 大脑（模型调用、工具执行循环）
- CodingSession 拥有 Coding 会话环境：资源、扩展、持久化、压缩和重试策略
- 两者通过事件流交互，继续以事件作为前端边界

关键特性：
1. 追加式持久化：使用 append-only 存储实现会话历史恢复和分支
2. 上下文压缩：自动/手动压缩机制管理长会话上下文
3. 扩展系统：支持 Skills、Prompt Templates、Extensions
4. 多模型支持：支持切换 Provider/Model
5. 会话树：支持分支（Branch）和回溯到历史节点
"""
...
from le_agent.provider_events import AssistantDoneEvent, AssistantErrorEvent, TextDeltaEvent
...
from le_agent.session.jsonl import entry_to_json_line
from le_agent.session.tree import SessionTreeError, path_to_entry
...
from le_agent_coding.branch_summary import summarize_branch_messages_with_model
from le_agent_coding.commands import CommandRegistry, CommandResult, create_default_command_registry
from le_agent_coding.context import discover_project_context_with_diagnostics
from le_agent_coding.context_window import (
...
from le_agent_coding.credentials import FileCredentialStore, credentials_path
from le_agent_coding.diagnostics import (
...
from le_agent_coding.extensions.runtime import ExtensionRuntime
from le_agent_coding.paths import LeAgentPaths
from le_agent_coding.prompt_templates import (
    PromptTemplate,
...
from le_agent_coding.provider_runtime import ClosableModelProvider, create_model_provider
from le_agent_coding.reload import CodingReloadSummary, ReloadCategorySummary
from le_agent_coding.resources import (
...
from le_agent_coding.session_manager import SessionManager
from le_agent_coding.session_stats import SessionStats, calculate_session_stats
from le_agent_coding.skills import Skill, expand_skill_command, load_skills_with_diagnostics
from le_agent_coding.system_prompt import (
    BuildSystemPromptOptions,
    ProjectContextFile,
    build_system_prompt,
)
from le_agent_coding.thinking import (
...
from le_agent_coding.tools import ImageSupportState, create_bash_tool, create_coding_tools
...
TREE_RUNNING_MESSAGE = "LeAgent is still working. Press Escape to interrupt before using /tree."
...
class SessionResources:
    """LeAgent-owned resources loaded around a coding session."""
...
# 资源、扩展、持久化、压缩和重试策略，同时继续以事件作为前端边界。
class CodingSession:
    """LeAgent's coding-agent environment wrapper.

    架构分层：
    - AgentHarness：拥有内存中的 Agent 大脑（模型调用、工具执行循环）
    - CodingSession：拥有 Coding 会话环境（资源、扩展、持久化、压缩、重试策略）

    主要职责：
    1. 会话生命周期管理：加载、恢复、分支、新建会话
    2. 持久化：使用追加式存储（SessionEntry）记录所有变更
    3. 资源管理：Skills、Prompt Templates、Context Files、Extensions
    4. 模型配置：Provider/Model 切换、Thinking Level 控制
    5. 上下文管理：自动/手动压缩、Token 估算
    6. 事件流：包装 Harness 事件流，添加应用层事件（压缩、重试等）

    事件边界：
    - 输入：前端通过 prompt() 提交用户消息
    - 输出：AsyncIterator[CodingSessionEvent] 事件流供前端消费
    """
    ...
    def __init__(
        ...
        *,
        ...
        """初始化 CodingSession 实例。

        参数说明：
            config: 会话配置（模型、工作目录、存储等）
            state: 从 SessionEntry 恢复的会话状态
            harness: Agent 核心（模型调用、工具执行）
            last_parent_id: 最后追加条目的父 ID，用于构建追加链
            skills: 加载的 Skill 列表（用于系统提示扩展）
            prompt_templates: 加载的 Prompt Template 列表
            context_files: 项目上下文文件列表
            resource_diagnostics: 资源加载诊断信息
            command_registry: 斜杠命令注册表
            pending_initial_entries: 首次持久化前待写入的初始条目
            extension_runtime: 扩展运行时环境
            image_support: 图像支持状态
        """
        self._config = config
        self._state = state
    ...
    async def load(cls, config: CodingSessionConfig) -> CodingSession:
        """从追加式存储加载 Coding Session。

        加载流程：
        1. 从存储读取所有 SessionEntry
        2. 若无条目，创建初始条目（SessionInfo、ModelChange、ThinkingLevelChange）
        3. 构建 SessionState（消息历史、活动分支）
        4. 加载资源（Skills、Prompt Templates、Context Files）
        5. 初始化/加载 Extension Runtime
        6. 创建 AgentHarness（绑定 Provider、Model、System Prompt、Tools）
        7. 构造 CodingSession 实例
        8. 绑定扩展运行时、附加事件监听器

        关键设计：
        - 历史与运行时配置解耦：从存储恢复状态，但使用当前配置构建 Harness
        - 扩展运行时复用：若 config 中已提供，直接使用；否则新建并加载
        - session_start 事件延迟：等待 Host 安装 UI Bridge 后再触发
        """
        # 先从追加式节点恢复活动分支，再用当前资源构造 Harness；历史与运行时配置因此解耦。
    ...
    def available_providers(self) -> tuple[str, ...]:
        """Return provider names LeAgent can call with available credentials."""
    ...
    def available_model_choices(self) -> tuple[ModelChoice, ...]:
        """Return provider/model choices LeAgent can call with available credentials."""
        if self._provider_settings is None:
    ...
    def tools(self) -> tuple[AgentTool, ...]:
        ...
        工具来源：
        1. 基础 Coding 工具（Bash、Read、Edit 等）
        2. 扩展提供的工具（通过 ExtensionRuntime 注册）
        3. 用户自定义工具（通过 config.tools 传入）

        工具组成流程：
        - load() 时创建基础工具集
        - ExtensionRuntime.compose_tools() 包装/扩展工具
        - 最终工具列表绑定到 AgentHarness

        注意：
        - 工具列表在运行时可通过 reload() 更新
        - 扩展可以包装基础工具以添加功能
        """
    ...
    def messages(self) -> tuple[AgentMessage, ...]:
        ...
        消息列表包含：
        - 系统提示（SystemMessage，通常是第一条）
        - 用户消息（UserMessage）
        - 助手消息（AssistantMessage）
        - 工具结果消息（ToolResultMessage）

        注意：
        - 这是从 AgentHarness 获取的当前状态
        - 修改消息历史应通过 Harness 的正式方法（如 replace_messages）
        - 每次访问返回新的元组（不变性保证）
        """
    ...
    async def branch_to_entry(
        ...
        *,
        ...
        这是会话树功能的核心：允许用户回到历史任意节点继续对话，
        原分支历史保留（形成树状结构），新消息从分支点继续。

        参数说明：
            entry_id: 要分支到的历史条目 ID
            summarize: 是否对被放弃的消息生成摘要
            custom_instructions: 摘要生成的自定义指令
            replace_instructions: 是否用 custom_instructions 完全替换默认指令

        执行流程：
        1. 检查 Harness 未在运行中
        2. 验证 entry_id 存在且可分支
        3. 若 summarize=True：
           - 收集分支点后的所有消息
           - 生成摘要并创建 BranchSummaryEntry
           - 摘要条目作为新分支的第一个节点
        4. 若选中的是 UserMessage，提供 input_prefill（方便编辑重发）
        5. 创建 LeafEntry 作为新活动叶子
        6. 刷新状态、替换 Harness 消息、同步配置

        持久化结构：
        - 原历史链保持不变
        - 新增 BranchSummaryEntry（可选）和 LeafEntry
        - LeafEntry 的 parent_id 指向分支目标或摘要条目
        """
    ...
    def skills(self) -> tuple[Skill, ...]:
        """返回当前会话加载的 Skill 列表（只读视图）。

        Skill 系统：
        - Skill 是可复用的提示词模板，可通过 /skill-name 命令触发
        - 加载来源：~/.le-agent/skills/ 目录下的 markdown 文件
        - 每个 Skill 包含名称、描述、内容（系统提示词片段）

        Skill 在系统提示词中的集成：
        - build_system_prompt() 将 Skill 内容注入系统提示词
        - 支持 {variable} 占位符替换
        - 可通过配置启用/禁用

        应用场景：
        - 代码审查 Skill：提供代码审查检查清单
        - 测试编写 Skill：指导如何编写测试
        - 重构 Skill：提供重构最佳实践
        """
        return self._skills
    ...
    def context_usage(self) -> ContextUsageEstimate:
        ...
        缓存机制：
        - 首次访问时计算并缓存
        - 消息变更（MessageEndEvent）、工具执行（ToolExecutionEndEvent）时失效缓存
        - 下次访问时重新计算
    ...
    def session_stats(self) -> SessionStats:
        ...
        统计内容：
        - 消息计数（用户消息、助手消息、工具调用等）
        - Token 使用（输入、输出、总计）
        - 估算成本（基于模型定价配置）
        - 会话持续时间

        计费计算：
        - 使用配置的 model_metadata 成本层级
        - 根据输入 token 数选择适用层级
        - 返回单位成本（每 1M tokens）
        """
    ...
    async def emit_pending_session_start(self) -> None:
        """发送由 `load` 延迟的 `session_start` 事件，每个会话一次。

        延迟机制：
        - load() 创建会话时设置 _session_start_pending = True
        - Host 在安装了 UI Bridge 后调用此方法
        - 确保 session_start 处理程序可以使用通知和对话框

        幂等性：
        - 首次调用：触发事件，清除 pending 标志
        - 后续调用：无操作（已启动的扩展运行时复用场景）

        应用场景：
        - Pi 的启动顺序：UI 在扩展初始化之前启动
        - TUI 启动流程：安装 UI 组件后触发扩展的 session_start
        """
    ...
    async def reload(self) -> CodingReloadSummary:
        """重新加载资源和扩展，带有生命周期边界等待。

        重新加载流程：
        1. 触发 session_shutdown(reason="reload") 事件
        2. 记录重新加载前的资源签名（Skills、Prompt Templates、Context Files 等）
        3. 重新加载资源（调用 _load_session_resources）
        4. 重新加载扩展（调用 _reload_extensions）
        5. 比较重新加载前后的资源变化
        6. 若系统提示输入发生变化，重建系统提示
        7. 触发 session_start(reason="reload") 事件
        8. 返回重新加载摘要

        生命周期边界：
        - 确保 session_shutdown 处理程序在 API 生成仍活跃时完成
        - 清除 UI 组件，使启动处理程序可以在干净的前端重新挂载
        - 等待新会话的 session_start 触发后再返回
    ...
    async def resume(self, session_id: str) -> str:
        ...
        使用场景：用户在 TUI 中选择"恢复会话"，切换到之前保存的其他会话。

        参数说明：
            session_id: 要恢复的会话 ID（来自 SessionManager 的索引）

        执行流程：
        1. 从 SessionManager 获取会话记录（cwd, model, provider 等）
        2. 使用目标会话的配置创建新的 CodingSession 实例
        3. 调用 _adopt_replacement() 采用新会话的状态
        4. 重新绑定扩展运行时到新会话对象
        5. 触发 session_start 事件

        关键设计：
        - 复用当前 ExtensionRuntime（扩展状态保持）
        - 替换内部状态（config, state, harness 等）
        - Provider/Model 可能随目标会话变化

        与 new_session() 的区别：
        - resume：加载已存在的会话历史
        - new_session：创建一个全新的空会话
        """
    ...
    async def new_session(self) -> str:
        """将当前会话状态替换为新的待处理未索引会话。

        使用场景：用户在 TUI 中选择"新建会话"，放弃当前会话开始新的对话。

        执行流程：
        1. 使用当前 Provider/Model 配置准备新会话记录
        2. 创建新的 CodingSession 实例（使用新会话的配置）
        3. 调用 _adopt_replacement() 采用新会话的状态
        4. 重新绑定扩展运行时到新会话对象
        5. 触发 session_start 事件
        ...
        关键设计：
        - 复用当前 ExtensionRuntime（扩展状态保持）
        - 保留当前 Provider/Model 配置
        - 新会话为空，无历史消息

        与 resume() 的区别：
        - new_session：创建全新的空会话
        - resume：加载已存在的会话历史
        """
        manager = self._config.session_manager
        if manager is None:
    ...
    async def compact(self, instructions: str | None = None) -> str:
        """手动压缩上下文：生成摘要并用摘要替换历史消息。

        压缩机制：
        1. 选择要压缩的消息范围（基于 token 阈值或启发式规则）
        2. 调用 LLM 生成这些消息的摘要
        3. 创建 CompactionEntry 包含摘要和元数据
        4. 更新消息历史：被压缩消息替换为摘要消息
        5. 持久化 CompactionEntry 和新的 LeafEntry

        参数说明：
            instructions: 自定义摘要生成指令（如"重点关注代码变更"）

        应用场景：
        - 手动触发：用户主动压缩会话
        - 自动触发：上下文接近模型限制时自动压缩
        - 溢出恢复：模型返回上下文溢出错误后的自动处理

        注意事项：
        - 压缩是单向操作，被替换的原始消息在活动中不再可见
        - 摘要质量影响后续对话效果
        """
    ...
    async def prompt(
        ...
        *,
        ...
        参数说明：
            content: 用户输入内容
            streaming_behavior: 流式行为（None/steer/follow_up）
            source: 触发来源（interactive/extension），用于 input hook
            custom_type: 自定义消息类型（扩展使用）
            details: 自定义消息元数据

        执行流程：
        1. 运行 input hooks（扩展可拦截或修改输入）
        2. 展开 prompt text（处理 Skill/Template 命令）
        3. 若 Harness 正在运行：
           - steer：发送引导消息到运行中的 Harness
           - follow_up：排队跟进消息
           - 否则报错
        4. 尝试自动压缩（若配置启用且上下文超限）
        5. 调用 Harness.prompt_message() 启动模型-工具循环
        6. 流式处理 Harness 事件：
           - MessageEndEvent：增量持久化消息
           - ToolExecutionEndEvent：刷新上下文缓存
           - AssistantMessage + stop_reason=error：记录诊断日志
        ...
        关键设计：
        - 事件流透传：大部分 Harness 事件直接透传给前端
        - 增量持久化：每条消息确认后立即落盘，不等待整轮结束
        - 错误恢复：上下文溢出时自动压缩并重试
        """
    ...
    async def continue_(self) -> AsyncIterator[CodingSessionEvent]:
        ...
        使用场景：
        1. 会话恢复后：从上次保存的状态继续对话
        2. 自动重试：上下文溢出压缩后继续对话（prompt() 内部调用）
        3. 队列消息：处理之前排队但尚未发送的消息

        与 prompt() 的区别：
        - prompt()：添加新用户消息并运行
        - continue_()：不添加新消息，从当前状态继续（如处理工具结果或恢复会话）
    ...
    async def _persist_loaded_interrupted_tool_repairs(self) -> None:
        ...
        Older LeAgent builds repaired interrupted tool-call transcripts only in the
        ...
        self._harness = AgentHarness(
            AgentHarnessConfig(
                ...
                tool_execution=self._harness.config.tool_execution,
            ),
    ...
    async def _persist_messages_since(self, persisted_count: int) -> int:
        ...
        for message in new_messages:
            # MessageEntry 推进活动分支；LeafEntry 额外记录当前可恢复的指针，旧节点从不重写。
...
def _resolve_export_destination(
    ...
    *,
    ...
    if destination is None:
        ...
        return cwd / f"le-agent-session.{format}"

    resolved = destination if destination.is_absolute() else cwd / destination
    if resolved.suffix:
        return resolved
    name = session_path.stem if session_path is not None else "le-agent-session"
    return default_session_export_artifact_path(
        Path(name),
...
def _session_export_title(session: CodingSession) -> str:
    ...
    if manager is not None and session_id is not None:
        ...
        if record is not None and record.title:
            return record.title
    return f"LeAgent session {session_id}" if session_id is not None else "LeAgent Session Export"
...
def _load_session_resources(
    resource_paths: LeAgentResourcePaths,
...
def default_session_path(cwd: Path) -> Path:
    """Return LeAgent's default user-home session path for a project cwd."""
    return LeAgentPaths().default_session_path(cwd)
