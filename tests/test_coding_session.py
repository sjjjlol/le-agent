...
from le_agent.messages import AssistantMessageDiagnostic, assistant_content
from le_agent.provider_events import AssistantErrorEvent
...
from le_agent_ai import CancellationToken, FakeProvider, ModelProvider, RuntimeModelLimits
...
from le_agent_coding import session as coding_session_module
from le_agent_coding.events import QueueUpdateEvent
from le_agent_coding.prompt_templates import PromptTemplate
from le_agent_coding.provider_config import ProviderModelMetadata
from le_agent_coding.session import _ordered_tree_entries, parse_terminal_command
...
async def test_session_export_defaults_to_cwd(tmp_path: Path) -> None:
    storage = JsonlSessionStorage(tmp_path / ".le-agent" / "sessions" / "session-1.jsonl")
    session = await CodingSession.load(_config(tmp_path, FakeProvider([]), storage))
...
async def test_prompt_logs_unexpected_agent_call_exception(tmp_path: Path) -> None:
    storage = JsonlSessionStorage(tmp_path / "session.jsonl")
    le_agent_paths = LeAgentPaths(
        home=tmp_path / "le-agent-home", agents_home=tmp_path / "agents-home"
    )
    session = await CodingSession.load(
        CodingSessionConfig(
            ...
            resource_paths=LeAgentResourcePaths(root=le_agent_paths.home, paths=le_agent_paths),
        )
    )
    ...
    log_path = le_agent_paths.agent_calls_log_path
...
async def test_tree_can_branch_from_first_user_message_before_assistant_response(
    ...
    with pytest.raises(RuntimeError, match="LeAgent is still working"):
...
async def test_session_persists_thinking_preference_for_new_sessions(tmp_path: Path) -> None:
    le_agent_paths = LeAgentPaths(home=tmp_path / ".le-agent")
    ...
    saved = load_provider_settings(le_agent_paths)
...
async def test_session_yields_expanded_custom_prompt_before_auto_naming(tmp_path: Path) -> None:
    ...
    session = await CodingSession.load(
        CodingSessionConfig(
            ...
            session_id=record.id,
            session_manager=manager,
            resource_paths=LeAgentResourcePaths(root=resources_root, agents_root=None),
        )
    )
...
async def test_system_command_shows_prompt_without_persisting_or_adding_context(
    ...
    assert result.message == "You are LeAgent."
...
async def test_session_reload_skips_provider_settings_refresh(
    ...
    def fail_load_provider_settings(paths: LeAgentPaths | None = None) -> ProviderSettings:
        del paths
...
async def test_session_provider_settings_reload_uses_session_paths(
    ...
    seen_paths: list[LeAgentPaths | None] = []

    def load_provider_settings(paths: LeAgentPaths | None = None) -> ProviderSettings:
    ...
    assert seen_paths == [le_agent_paths]
...
async def test_session_switch_uses_session_credential_store(
    ...
    FileCredentialStore(le_agent_paths.home / "credentials.json").set("openai", "stored-key")
    ...
    assert credential_store_paths == [le_agent_paths.home / "credentials.json"]
...
async def test_session_toggles_and_cycles_scoped_models(
    ...
    saved = json.loads((le_agent_paths.home / "providers.json").read_text(encoding="utf-8"))
...
async def test_session_toggle_scoped_model_preserves_newer_provider_file_changes(
    ...
    save_provider_settings(newer_settings, le_agent_paths)
    session = await CodingSession.load(
        CodingSessionConfig(
            provider=FakeProvider([]),
            model="qwen",
    ...
    saved = coding_session_module.load_provider_settings(le_agent_paths)
...
async def test_session_set_model_persists_default_provider_model(
    ...
    isolate_home(monkeypatch, tmp_path)
    le_agent_paths = LeAgentPaths(home=tmp_path / ".le-agent", agents_home=tmp_path / ".agents")
    provider_config = OpenAICompatibleProviderConfig(
        name="openai",
...
async def test_session_set_model_preserves_newer_provider_file_changes(
    ...
    saved = coding_session_module.load_provider_settings(le_agent_paths)
    assert saved.get_provider("openai").default_model == "gpt-5-mini"
    assert saved.get_provider("openrouter").headers == {"X-Title": "LeAgent"}
    assert saved.scoped_models == (
