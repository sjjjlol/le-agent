...
import le_agent_coding.provider_config as provider_config
...
from le_agent_coding.provider_catalog import ModelCostTier
...
def test_stale_preferences_cannot_restore_removed_codex_alias(tmp_path: Path) -> None:
    ...
    le_agent_home.mkdir()
    (le_agent_home / "providers.json").write_text(
        json.dumps(
            {
                "schema_version": 2,
    ...
    settings = load_provider_settings(LeAgentPaths(home=le_agent_home))
...
def test_load_provider_settings_accepts_provider_preferences_with_user_catalog(
    ...
    le_agent_home = tmp_path / ".le-agent"
    le_agent_home.mkdir()
    (le_agent_home / "catalog.toml").write_text(
        """
schema_version = 1
...
def test_save_and_load_provider_settings_round_trip(
    ...
    assert path == tmp_path / ".le-agent" / "providers.json"
...
def test_kimi_k3_maps_xhigh_thinking_to_max(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    settings = load_provider_settings(LeAgentPaths(home=Path("/missing")))
...
""".strip(),
    ...
    migrated = json.loads((le_agent_home / "providers.json").read_text())
    assert migrated["schema_version"] == 2
    assert "providers" not in migrated
    assert (le_agent_home / "providers.json.bak").exists()
    assert not (le_agent_home / "catalog.toml").exists()
...
def test_load_provider_settings_migrates_custom_provider_to_catalog(
    ...
    (le_agent_home / "providers.json").write_text(json.dumps(original), encoding="utf-8")
    ...
    assert provider.headers == {"X-Test": "yes"}
    assert json.loads((le_agent_home / "providers.json.bak").read_text()) == original
    migrated = json.loads((le_agent_home / "providers.json").read_text())
    assert migrated["schema_version"] == 2
    assert migrated["provider_preferences"]["local"]["default_model"] == "qwen"
    catalog = (le_agent_home / "catalog.toml").read_text()
...
def test_legacy_migration_aborts_before_changes_when_backup_fails(
    ...
    settings_path = le_agent_home / "providers.json"
    ...
    with pytest.raises(PermissionError, match="backup denied"):
        load_provider_settings(LeAgentPaths(home=le_agent_home))

    assert json.loads(settings_path.read_text()) == original
    assert not (le_agent_home / "providers.json.bak").exists()
    assert not (le_agent_home / "catalog.toml").exists()
...
""".strip(),
    ...
    store = FileCredentialStore(le_agent_home / "credentials.json")
