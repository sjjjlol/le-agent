...
from le_agent_coding.catalog_loader import (
    CatalogError,
    builtin_catalog,
...
from le_agent_coding.provider_config import load_provider_settings
...
def _write_user_catalog(le_agent_home: Path, body: str) -> LeAgentPaths:
    paths = LeAgentPaths(home=le_agent_home)
    le_agent_home.mkdir(parents=True, exist_ok=True)
...
def test_user_catalog_adds_new_provider(tmp_path: Path) -> None:
    paths = _write_user_catalog(tmp_path / ".le-agent", VALID_PROVIDER)
...
""",
    ...
    entry = next(e for e in effective_catalog(paths) if e.name == "openai")
    assert entry.models[0] == "gpt-next"
    assert "gpt-5.5" in entry.models
    assert entry.default_model == "gpt-next"
    assert entry.context_windows is not None
    assert entry.context_windows["gpt-next"] == 500_000
    assert entry.context_windows["gpt-5.5"] == 272_000
...
""",
    ...
    entry = next(e for e in effective_catalog(paths) if e.name == "huggingface")
    metadata = entry.model_metadata["MiniMaxAI/MiniMax-M3"]
    ...
    reloaded = next(e for e in effective_catalog(paths) if e.name == "huggingface")
    assert (
        model_cost_for_input_tokens(reloaded.model_metadata["MiniMaxAI/MiniMax-M3"], 400_001)
        == long_context_cost
    )
...
def test_user_catalog_rejects_unknown_keys(tmp_path: Path) -> None:
    paths = _write_user_catalog(
        tmp_path / ".le-agent", VALID_PROVIDER.replace("docs_url", "docs_ur1")
    )
...
def test_user_catalog_rejects_empty_and_coerced_values(
    ...
    paths = _write_user_catalog(tmp_path / ".le-agent", body)
...
def test_user_catalog_rejects_bad_kind(tmp_path: Path) -> None:
    paths = _write_user_catalog(
        tmp_path / ".le-agent", VALID_PROVIDER.replace("openai-compatible", "grpc")
    )
...
def test_user_catalog_rejects_malformed_toml(tmp_path: Path) -> None:
    paths = _write_user_catalog(tmp_path / ".le-agent", "[[providers]\nname =")
...
def test_user_catalog_provider_appears_with_existing_settings_file(tmp_path: Path) -> None:
    paths = _write_user_catalog(tmp_path / ".le-agent", VALID_PROVIDER)
    (tmp_path / ".le-agent" / "providers.json").write_text(
