...
from le_agent_coding import updater
from le_agent_coding.updater import detect_install_method, update_le_agent
...
def test_update_le_agent_uses_uv_tool_for_uv_owned_tool_environment(tmp_path: Path) -> None:
    (tmp_path / "uv-receipt.toml").touch()
    ...
    assert result.command == ("uv", "tool", "install", "le-agent@0.2.4")
    assert calls == [("uv", "tool", "install", "le-agent@0.2.4")]


def test_update_le_agent_hands_windows_uv_tool_update_to_waiting_process(tmp_path: Path) -> None:
    (tmp_path / "uv-receipt.toml").touch()
    ...
    def runner(*args: object, **kwargs: object) -> CompletedProcess[str]:
        del args, kwargs
        raise AssertionError("uv must not run in the live LeAgent process")
    ...
    assert json.loads(base64.b64decode(detached_command[-1])) == {
        "executable": "uv",
        "arguments": '"tool" "install" "le-agent@0.2.4"',
    }
...
def test_windows_helper_wait_failure_is_fail_closed(tmp_path: Path, powershell: str | None) -> None:
    ...
    assert "Could not wait for LeAgent process" in log_path.read_text(encoding="utf-8-sig")


def test_update_le_agent_reports_uv_latest_version_lookup_failure(tmp_path: Path) -> None:
    (tmp_path / "uv-receipt.toml").touch()
    ...
    assert result.succeeded is False
    assert result.failures == ("Could not determine the latest LeAgent version from PyPI.",)


def test_update_le_agent_uses_pipx_for_pipx_owned_environment(tmp_path: Path) -> None:
    (tmp_path / "pipx_metadata.json").touch()
    ...
    assert result.command == ("pipx", "upgrade", "le-agent")


def test_update_le_agent_reuses_uv_pip_for_uv_installed_distribution(tmp_path: Path) -> None:
    result = update_le_agent(
        runner=_success,
...
def test_update_le_agent_uses_current_environment_pip_for_pip_install(tmp_path: Path) -> None:
    result = update_le_agent(
        runner=_success,
        python_executable="/env/bin/python",
        environment_prefix=tmp_path,
...
def test_update_le_agent_does_not_fall_back_when_owner_update_fails(tmp_path: Path) -> None:
    (tmp_path / "uv-receipt.toml").touch()
    calls: list[tuple[str, ...]] = []
    ...
    assert result.succeeded is False
    assert result.failures == ("uv tool install le-agent@0.2.4: uv failed",)
    assert calls == [("uv", "tool", "install", "le-agent@0.2.4")]


def test_update_le_agent_refuses_direct_url_install(tmp_path: Path) -> None:
    result = update_le_agent(
        runner=_success,
        environment_prefix=tmp_path,
        direct_url="file:///checkout/le-agent",
        installer="uv",
        inspect_distribution=False,
    )

    assert result.succeeded is False
    assert "original source: file:///checkout/le-agent" in result.failures[0]


def test_update_le_agent_refuses_conda_or_pixi_environment(tmp_path: Path) -> None:
...
def test_update_le_agent_refuses_unknown_installer(tmp_path: Path) -> None:
    result = update_le_agent(
        runner=_success,
        environment_prefix=tmp_path,
