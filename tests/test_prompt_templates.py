...
def test_load_prompt_templates_missing_directory_returns_empty(tmp_path: Path) -> None:
    assert load_prompt_templates(LeAgentResourcePaths(root=tmp_path, agents_root=None)) == []
...
def test_load_prompt_templates_from_markdown_files(tmp_path: Path) -> None:
    ...
    templates = load_prompt_templates(LeAgentResourcePaths(root=tmp_path, agents_root=None))
...
def test_load_prompt_templates_with_diagnostics_reports_overrides(tmp_path: Path) -> None:
    ...
    agents_home = tmp_path / "home" / ".agents"
    cwd = tmp_path / "project"
    (le_agent_home / "prompts").mkdir(parents=True)
    (le_agent_home / "prompts" / "review.md").write_text("User LeAgent review", encoding="utf-8")
    (cwd / ".le-agent" / "prompts").mkdir(parents=True)
    (cwd / ".le-agent" / "prompts" / "review.md").write_text(
        "Project LeAgent review",
        encoding="utf-8",
    )
    ...
    assert templates[0].path == cwd / ".le-agent" / "prompts" / "review.md"
