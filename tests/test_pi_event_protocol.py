...
def test_le_agent_does_not_import_le_agent_ai() -> None:
    for path in Path("src/le_agent").rglob("*.py"):
        source = path.read_text(encoding="utf-8")
        assert "from le_agent_ai" not in source, path
        assert "import le_agent_ai" not in source, path


def test_le_agent_ai_reexports_canonical_event_classes() -> None:
    from le_agent_ai.events import TextDeltaEvent as public
