...
from le_agent_coding.tui.terminal_notification import (
...
def test_desktop_notification_sequence_uses_terminal_protocol() -> None:
    assert (
        desktop_notification_sequence("LeAgent turn finished", environ={"TERM_PROGRAM": "ghostty"})
        == "\x1b]9;LeAgent turn finished\x07"
    )
    assert (
        desktop_notification_sequence("LeAgent turn finished", environ={"TERM": "xterm-kitty"})
        == "\x1b]99;;LeAgent turn finished\x1b\\"
    )
    assert (
        desktop_notification_sequence(
            "LeAgent turn finished", environ={"TERM_PROGRAM": "Apple_Terminal"}
        )
        is None
    )
...
def test_terminal_notification_controller_writes_configured_sequence() -> None:
    ...
    assert desktop_writes == ["\x1b]9;LeAgent turn finished\x07"]
