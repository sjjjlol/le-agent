...
from le_agent.messages import assistant_content
from le_agent.provider_events import TextDeltaEvent, ThinkingDeltaEvent
from le_agent_coding.catalog_loader import user_catalog_path
from le_agent_coding.commands import CommandResult
from le_agent_coding.credentials import FileCredentialStore, OAuthCredential
from le_agent_coding.events import (
    AgentSettledEvent,
    CodingSessionEvent,
    CompactionEndEvent,
...
from le_agent_coding.prompt_templates import PromptTemplate
...
from le_agent_coding.session_manager import CodingSessionRecord
from le_agent_coding.session_stats import SessionStats
from le_agent_coding.skills import Skill, format_skill_invocation
from le_agent_coding.system_prompt import ProjectContextFile
from le_agent_coding.tools import create_coding_tools
from le_agent_coding.tui import app as tui_app
from le_agent_coding.tui.app import (
    ...
    _textual_theme_for_le_agent_theme,
    _theme_css_variables,
    _TuiExtensionUiBridge,
    _visible_completion_state,
)
from le_agent_coding.tui.autocomplete import CompletionItem, CompletionState
...
from le_agent_coding.tui.terminal_notification import TerminalNotificationController
from le_agent_coding.tui.terminal_title import TerminalTitleController
from le_agent_coding.tui.widgets import (
    TRANSCRIPT_WINDOW_ITEMS,
    TRANSCRIPT_WINDOW_OVERSCAN_ITEMS,
    CompactSessionInfo,
    LeAgentMarkdownBlock,
...
class FakeSession:
    def __init__(self, messages=(), events=()) -> None:
        ...
        self.system_prompt = "You are LeAgent."
...
def _visible_footer_bindings(app: LeAgentTuiApp) -> dict[str, str]:
...
def test_comma_list_limits_by_rendered_lines_instead_of_item_count() -> None:
    ...
    narrow_console.print(_comma_list(items, empty="Empty", theme=LE_AGENT_DARK_THEME))
    ...
    wide_console.print(_comma_list(items, empty="Empty", theme=LE_AGENT_DARK_THEME))
...
def test_session_sidebar_brand_includes_current_version() -> None:
    ...
    console.print(_sidebar_brand(theme=LE_AGENT_DARK_THEME))

    assert "LeAgent  0.2.0" in console.export_text()
...
def test_session_sidebar_uses_prominent_title_and_accented_section_headers() -> None:
    ...
    assert str(session_name.renderable.style) == f"bold {LE_AGENT_DARK_THEME.accent}"
    assert activity_header.left == 1
    assert str(activity_header.renderable.style) == f"bold {LE_AGENT_DARK_THEME.prompt_text}"
    assert str(activity_content.renderable.style) == LE_AGENT_DARK_THEME.completion_description
    assert " context" in output
...
def test_compact_session_info_styles_provider_as_metadata() -> None:
    ...
    assert (
        f"\x1b[{_style_color_escape(LE_AGENT_DARK_THEME.completion_description)}mopenai" in output
    )
    assert f"\x1b[{_style_color_escape(LE_AGENT_DARK_THEME.prompt_text)}m:fake-model" in output


def test_compact_session_info_styles_parent_path_as_metadata() -> None:
    cwd = _styled_cwd(Path("/workspace/project"), theme=LE_AGENT_DARK_THEME)

    assert cwd.plain == "/workspace/project (--)"
    assert str(cwd.spans[0].style) == LE_AGENT_DARK_THEME.completion_description
    assert str(cwd.spans[1].style) == LE_AGENT_DARK_THEME.prompt_text
    assert str(cwd.spans[2].style) == LE_AGENT_DARK_THEME.completion_description
...
def test_tui_state_compacts_expanded_skill_messages() -> None:
    skill = Skill(
        name="review",
        path=Path("/workspace/.le-agent/skills/review.md"),
        content="# Review\nFull noisy instructions.",
                ...
                content=assistant_content(
                    ...
                    [
                        ToolCall(
                            id="call-1",
                            name="read",
                            arguments={"path": "/workspace/.le-agent/skills/review.md"},
                        )
                    ],
                )
            ),
...
def test_dark_theme_markdown_code_uses_aqua_highlight() -> None:
    console = Console(record=True, width=80)
    console.print(render_chat_item(ChatItem(role="assistant", text="Use `le-agent` here.")))
...
def test_assistant_markdown_titles_use_highlight_color_and_left_alignment() -> None:
    ...
    assert _style_color_escape(LE_AGENT_DARK_THEME.markdown_heading) in output
...
def test_dark_theme_markdown_bullets_use_theme_bullet_color() -> None:
    ...
    assert _style_color_escape(LE_AGENT_DARK_THEME.markdown_bullet) in output
...
def test_markdown_tables_use_highlight_color_for_headers() -> None:
    ...
    assert _style_color_escape(LE_AGENT_DARK_THEME.accent) in output
...
async def test_textual_markdown_widget_uses_theme_link_style() -> None:
    ...
    async with app.run_test() as pilot:
        ...
        block = app.query_one(LeAgentMarkdownBlock)
    ...
    assert markdown.le_agent_link_style == LE_AGENT_DARK_THEME.markdown_link
...
def test_textual_markdown_uses_theme_highlight_and_aqua_inline_code() -> None:
    variables = _theme_css_variables(LE_AGENT_LIGHT_THEME)

    assert variables["le-agent-markdown-highlight"] == LE_AGENT_LIGHT_THEME.markdown_heading
    assert variables["le-agent-markdown-table-header"] == LE_AGENT_LIGHT_THEME.markdown_table_header
    assert variables["le-agent-markdown-table-border"] == LE_AGENT_LIGHT_THEME.markdown_table_border
    assert variables["le-agent-markdown-inline-code"] == LE_AGENT_LIGHT_THEME.markdown_inline_code
    assert (
        variables["le-agent-markdown-code-block-background"]
        == LE_AGENT_LIGHT_THEME.markdown_code_block_background
    )
    assert variables["le-agent-markdown-link"] == LE_AGENT_LIGHT_THEME.markdown_link
    assert variables["le-agent-markdown-bullet"] == LE_AGENT_LIGHT_THEME.markdown_bullet
...
def test_light_theme_markdown_code_uses_aqua_without_background() -> None:
    ...
    console.print(
        render_chat_item(
            ChatItem(role="assistant", text="Use `le-agent` here."),
            theme=LE_AGENT_LIGHT_THEME,
        )
    )
...
def test_pending_tool_invocation_uses_tool_accent_color() -> None:
    ...
    console.print(
        _transcript_plain_body_text(
            item,
            text=item.text,
            body_style=LE_AGENT_DARK_THEME.role_styles["tool"].body,
            theme=LE_AGENT_DARK_THEME,
        )
    )
...
async def test_streaming_transcript_applies_role_foreground() -> None:
    ...
    async with app.run_test(size=(60, 20)) as pilot:
        ...
        thinking_fg, _ = _split_rich_style_colors(LE_AGENT_DARK_THEME.role_styles["thinking"].body)
        assistant_fg, _ = _split_rich_style_colors(
            LE_AGENT_DARK_THEME.role_styles["assistant"].body
        )
        ...
        await transcript.append_thinking_delta(
            "reasoning", theme=LE_AGENT_DARK_THEME, show_thinking=True
        )
        await pilot.pause()
        thinking = next(
        ...
        await transcript.append_assistant_delta("answer", theme=LE_AGENT_DARK_THEME)
        await pilot.pause()
        assistant = next(
...
async def test_tool_completion_updates_row_without_redrawing_history() -> None:
    app = LeAgentTuiApp(FakeSession(messages=[UserMessage(content="earlier")]))
...
async def test_long_transcript_mounts_bounded_latest_window_and_pages_earlier(
    ...
    from le_agent_coding.tui import widgets as tui_widgets
...
async def test_tool_transcript_uses_native_markdown_without_custom_selection_painting() -> None:
    app = LeAgentTuiApp(FakeSession(messages=[]))
    item = ChatItem(
        role="tool",
        text="→ read README.md",
        tool_result_text="✓ read\nREADME contents",
...
def test_le_agent_markdown_block_is_not_selectable_until_mounted() -> None:
    markdown = TextualMarkdown("example")
    block = LeAgentMarkdownBlock(
        markdown,
...
@pytest.mark.anyio
async def test_le_agent_markdown_block_remains_selectable_after_mount() -> None:
    app = LeAgentTuiApp(
        FakeSession([AssistantMessage(content="Read [docs](https://example.com).")]),
    )
...
async def test_tui_sidebar_is_visible_on_medium_windows() -> None:
    ...
    async with app.run_test(size=(120, 40)):
        ...
        assert sidebar.styles.background == Color.parse(LE_AGENT_DARK_THEME.prompt_background)
...
async def test_tui_sidebar_shows_on_left_when_configured() -> None:
    app = LeAgentTuiApp(FakeSession(), tui_settings=TuiSettings(sidebar_position="left"))
...
async def test_tui_sidebar_is_hidden_when_off() -> None:
    app = LeAgentTuiApp(FakeSession(), tui_settings=TuiSettings(sidebar_position="off"))
...
async def test_tui_sidebar_right_still_hides_on_small_windows() -> None:
    app = LeAgentTuiApp(FakeSession(), tui_settings=TuiSettings(sidebar_position="right"))
...
async def test_tui_transcript_code_block_scrollbar_matches_overflow(
    ...
    app = LeAgentTuiApp(FakeSession(messages=[AssistantMessage(content=f"```python\n{code}\n```")]))
...
@pytest.mark.parametrize(
    "theme",
    [LE_AGENT_DARK_THEME, LE_AGENT_LIGHT_THEME, HIGH_CONTRAST_THEME],
    ids=lambda theme: theme.name,
)
async def test_transcript_code_fence_uses_theme_background(theme: TuiTheme) -> None:
    """Code fences keep LeAgent's themed background for every built-in theme.

    Regression test: Textual's built-in `MarkdownFence:light` rule outranks
    LeAgent's plain `ThemedMarkdownWidget MarkdownFence` selector, which used to
...
def test_tui_app_uses_configured_theme_css_variables() -> None:
    app = LeAgentTuiApp(FakeSession(), tui_settings=TuiSettings(theme="high-contrast"))

    variables = app.get_theme_variable_defaults()

    assert variables["le-agent-screen-background"] == "#000000"
    assert variables["le-agent-prompt-background"] == "#1a1a1a"
    assert variables["le-agent-prompt-border"] == "#00ff66"
...
def test_tui_app_registers_and_applies_custom_theme() -> None:
    from le_agent_coding.tui.themes import (
        THEME_COLOR_FIELDS,
        TRANSCRIPT_ROLES,
    ...
    try:
        app = LeAgentTuiApp(FakeSession(), tui_settings=TuiSettings(theme="midnight"))

        assert app.theme == "midnight"
        assert app.get_theme_variable_defaults()["le-agent-screen-background"] == "#123456"
    finally:
        set_custom_tui_themes({})


def test_tui_app_falls_back_to_le_agent_dark_when_theme_is_missing(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    isolate_home(monkeypatch, tmp_path)

    app = LeAgentTuiApp(FakeSession(), tui_settings=TuiSettings(theme="missing-theme"))

    assert app.theme == "le-agent-dark"
...
def test_tui_app_uses_light_theme_css_variables() -> None:
    app = LeAgentTuiApp(FakeSession(), tui_settings=TuiSettings(theme="le-agent-light"))

    variables = app.get_theme_variable_defaults()

    assert variables["le-agent-screen-background"] == "#ffffff"
    assert variables["le-agent-chrome-background"] == "#f3f4f6"
    assert variables["le-agent-muted-text"] == "#475569"
    assert variables["le-agent-prompt-background"] == "#f8fafc"
    assert variables["le-agent-prompt-border"] == "#2563eb"
...
def test_tui_app_registers_only_le_agent_themes_with_textual() -> None:
    app = LeAgentTuiApp(FakeSession())

    assert tuple(app.available_themes) == ("le-agent-dark", "le-agent-light", "high-contrast")


def test_textual_theme_mapping_uses_le_agent_theme_values() -> None:
    textual_theme = _textual_theme_for_le_agent_theme("le-agent-light")

    assert textual_theme.name == "le-agent-light"
    assert textual_theme.primary == LE_AGENT_LIGHT_THEME.accent
    assert textual_theme.dark is False
    assert (
        textual_theme.variables["le-agent-screen-background"]
        == LE_AGENT_LIGHT_THEME.screen_background
    )


def test_le_agent_dark_theme_uses_aqua_as_its_shared_accent() -> None:
...
def test_autocomplete_and_picker_share_theme_highlight(theme: TuiTheme) -> None:
    ...
    assert variables["le-agent-highlight-background"] == theme.highlight_background
    assert variables["le-agent-highlight-text"] == theme.highlight_text


def test_le_agent_light_theme_uses_light_chat_backgrounds() -> None:
    theme = TuiSettings(theme="le-agent-light").resolved_theme
...
async def test_tui_app_updates_terminal_title_for_running_and_named_session() -> None:
    ...
    async with app.run_test():
        assert writes[-1] == "\x1b]0;le-agent | build notes\x07"
        ...
        app._refresh_chrome()
        assert writes[-1] == "\x1b]0;⠙ le-agent | ship notes\x07"

        app.adapter.apply(AgentEndEvent())
        app._refresh()
        assert writes[-1] == "\x1b]0;le-agent | ship notes\x07"

    assert writes[-1] == "\x1b]0;le-agent\x07"
...
async def test_tui_app_notifies_when_agent_settles_while_unfocused() -> None:
    ...
    app = LeAgentTuiApp(SettledSession(), tui_settings=TuiSettings(turn_notification="desktop"))
    writes: list[str] = []
    ...
    async with app.run_test():
        ...
        assert writes == ["\x1b]9;LeAgent turn finished\x07"]
...
async def test_tui_app_updates_terminal_title_after_auto_session_naming() -> None:
    ...
    app = LeAgentTuiApp(AutoNamingSession())
    writes: list[str] = []
    app._terminal_title = TerminalTitleController(enabled=True, writer=writes.append)
    ...
    async with app.run_test():
        ...
        assert "\x1b]0;le-agent | Debug login\x07" in writes
        ...
        assert writes[-1] == "\x1b]0;le-agent | Debug login\x07"
...
@pytest.mark.anyio
async def test_textual_theme_change_persists_le_agent_theme(
    ...
    async with app.run_test() as pilot:
        app.theme = "le-agent-light"
        await pilot.pause()

    assert app.tui_settings.theme == "le-agent-light"
    assert tui_settings_path().read_text(encoding="utf-8").find('"theme": "le-agent-light"') != -1
...
async def test_tui_app_theme_command_opens_picker_and_persists_selection(
    ...
    async with app.run_test() as pilot:
        ...
        assert app.tui_settings.theme == "le-agent-light"
        assert app.theme == "le-agent-light"
        assert (
            tui_settings_path().read_text(encoding="utf-8").find('"theme": "le-agent-light"') != -1
        )
        assert app.get_theme_variable_defaults()["le-agent-screen-background"] == "#ffffff"
...
async def test_theme_picker_highlight_uses_theme_selection_palette(theme: TuiTheme) -> None:
    theme_names = (LE_AGENT_DARK_THEME.name, LE_AGENT_LIGHT_THEME.name, HIGH_CONTRAST_THEME.name)
    app = LeAgentTuiApp(FakeSession(), tui_settings=TuiSettings(theme=theme.name))
...
async def test_extension_select_dialog_returns_choice() -> None:
    app = LeAgentTuiApp(FakeSession())  # type: ignore[arg-type]
...
async def test_restored_tool_calls_render_via_render_call() -> None:
    ...
    app = LeAgentTuiApp(session)  # type: ignore[arg-type]
...
async def test_tui_app_theme_command_argument_updates_theme_and_persists(
    ...
    async with app.run_test() as pilot:
        prompt = app.query_one("#prompt")
        prompt.value = "/theme le-agent-light"
        await pilot.press("enter")
...
async def test_tui_app_shows_working_state_during_manual_compaction() -> None:
    ...
    async with app.run_test() as pilot:
        ...
        assert indicator.render().plain != "L"
        assert titles[-1] == "\x1b]0;⠋ le-agent | build notes\x07"
...
async def test_tui_app_clears_working_state_when_manual_compaction_fails() -> None:
    ...
    app = LeAgentTuiApp(FailingCompactSession(messages=[UserMessage(content="Earlier")]))
    notifications: list[str] = []
...
async def test_tui_app_clears_working_state_when_compaction_is_cancelled() -> None:
    ...
    app = LeAgentTuiApp(SlowCompactSession(messages=[UserMessage(content="Earlier")]))
...
async def test_tui_app_notifies_once_for_turn_with_automatic_compaction() -> None:
    ...
    app = LeAgentTuiApp(AutoCompactSession(), tui_settings=TuiSettings(turn_notification="desktop"))
    writes: list[str] = []
    app._terminal_notification = TerminalNotificationController(
        "desktop",
        enabled=True,
...
async def test_tui_app_blocks_tree_picker_while_agent_is_running() -> None:
    ...
    async with app.run_test() as pilot:
        ...
        assert notifications == [
            "LeAgent is still working. Press Escape to interrupt before using /tree."
        ]
...
async def test_tui_app_tree_picker_branches_with_summary() -> None:
    ...
    async with app.run_test() as pilot:
        ...
        assert str(rendered_labels[0].spans[0].style) == _style_rgb(LE_AGENT_DARK_THEME.accent)
        assert str(rendered_labels[3].spans[0].style) == _style_rgb(
            LE_AGENT_DARK_THEME.highlight_text
        )
        ...
        assert str(left_label.spans[0].style) == _style_rgb(LE_AGENT_DARK_THEME.highlight_text)
        assert str(right_label.spans[0].style) == _style_rgb(LE_AGENT_DARK_THEME.accent)
        await pilot.press("s")
        await pilot.pause()
...
async def test_tui_app_session_modal_auto_copies_selected_text(
    ...
    app = LeAgentTuiApp(FakeSession(), tui_settings=TuiSettings(auto_copy_selection=False))
    copied: list[str] = []
    monkeypatch.setattr(app, "copy_to_clipboard", copied.append)
...
async def test_tui_login_saves_provider_key(
    ...
    assert (tmp_path / ".le-agent" / "credentials.json").read_text(encoding="utf-8")
...
async def test_tui_login_openai_codex_saves_oauth_credentials(
    ...
    credentials = (tmp_path / ".le-agent" / "credentials.json").read_text(encoding="utf-8")
...
async def test_tui_login_custom_provider_writes_catalog_and_preferences(
    ...
    paths = LeAgentPaths(home=tmp_path / ".le-agent")
    ...
    assert FileCredentialStore(tmp_path / ".le-agent" / "credentials.json").get("nebius") == (
        "stored-nebius-key"
    )
    assert session.provider_reload_count == 1
...
async def test_tui_login_preserves_existing_scoped_models_and_providers(
    ...
    isolate_home(monkeypatch, tmp_path)
    le_agent_home = tmp_path / ".le-agent"
    ...
    assert FileCredentialStore(le_agent_home / "credentials.json").get("openrouter") == (
...
async def test_tui_logout_removes_stored_api_key(
    ...
    isolate_home(monkeypatch, tmp_path)
    credential_path = tmp_path / ".le-agent" / "credentials.json"
...
async def test_tui_logout_opens_stored_credential_provider_picker(
    ...
    isolate_home(monkeypatch, tmp_path)
    FileCredentialStore(tmp_path / ".le-agent" / "credentials.json").set("openai", "stored-key")
    app = LeAgentTuiApp(FakeSession())
...
async def test_tui_login_api_key_opens_api_provider_picker() -> None:
    ...
    async with app.run_test() as pilot:
        ...
        assert isinstance(app.screen, LoginScreen)
        assert app.screen.provider.name == "deepseek"
...
async def test_tui_app_up_arrow_preserves_non_empty_prompt_movement() -> None:
    app = LeAgentTuiApp(FakeSession(messages=[UserMessage(content="remembered prompt")]))
...
async def test_tui_prompt_ctrl_c_clears_text() -> None:
    app = LeAgentTuiApp(FakeSession(messages=(UserMessage(content="User prompt"),)))
...
async def test_tui_prompt_worker_refreshes_directly() -> None:
    app = LeAgentTuiApp(FakeSession(events=[AgentStartEvent(), AgentEndEvent()]))
    refreshes = 0
    ...
    class ErrorSession(FakeSession):
        def __init__(self) -> None:
            ...
            self.last_diagnostic_log_path = (
                tmp_path / "le-agent-home" / "logs" / "agent-calls.jsonl"
            )
...
async def test_tui_app_shows_startup_update_notice_first_in_bright_yellow() -> None:
    ...
    app = LeAgentTuiApp(
        session,
        startup_update_notice="LeAgent 0.2.0 is available",
        startup_notices=("LeAgent updated to 0.2.0",),
    )
    notifications: list[tuple[str, str | None]] = []
    ...
    async with app.run_test() as pilot:
        ...
        transcript = app.query_one("#transcript", TranscriptView)
        assert [line.text for line in transcript.lines] == [
            "LeAgent 0.2.0 is available",
            "LeAgent updated to 0.2.0",
            "Earlier prompt",
        ]
...
async def test_tui_app_runs_initial_prompt() -> None:
    ...
    app = LeAgentTuiApp(session, initial_prompt="explain this repo")
...
async def test_run_tui_app_falls_back_to_first_credentialed_provider(
    ...
    monkeypatch.setattr(tui_app, "LeAgentTuiApp", FakeApp)
...
async def test_run_tui_app_opens_when_provider_login_is_missing(
    ...
    class FakeApp:
        def __init__(self, session: str, **kwargs: object) -> None:
            ...
            assert "LeAgent 0.2.0 is available" not in message
            ...
            assert "LeAgent 0.2.0 is available" in notices
    ...
    await tui_app.run_tui_app(
        ...
        model=None,
        session_manager=FakeManager(),
        startup_notice="LeAgent 0.2.0 is available",
    )
...
# These drive the generic widget-hosting seam on the real LeAgentTuiApp via the
...
def _component_bridge(app: LeAgentTuiApp) -> _TuiExtensionUiBridge:
...
async def test_component_key_interceptor_preempts_priority_binding() -> None:
    ...
    LeAgentTuiApp.on_event, ahead of that priority check, so it can own those keys.
    """
    app = LeAgentTuiApp(FakeSession())
