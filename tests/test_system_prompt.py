...
from le_agent import AgentTool, AgentToolResult
from le_agent_coding import Skill
...
def test_default_prompt_includes_tools_guidelines_date_and_cwd(tmp_path: Path) -> None:
    ...
    assert "You are an expert coding assistant operating inside LeAgent" in prompt
    ...
    assert "LeAgent documentation (read only when the user asks about LeAgent itself" in prompt
...
def test_custom_prompt_replaces_default_but_keeps_append_context_and_date(tmp_path: Path) -> None:
    ...
    assert "LeAgent documentation" not in prompt
