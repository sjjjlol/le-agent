...
from le_agent_coding.commands import CommandRegistry, SlashCommand, create_default_command_registry
...
from le_agent_coding.session import ModelChoice
...
def test_session_command_includes_session_details(tmp_path: Path) -> None:
    ...
    assert "Tools: 5" in result.message
...
def test_login_command_lists_auth_aliases_for_unknown_provider(tmp_path: Path) -> None:
    ...
    assert "anthropic" not in result.message
