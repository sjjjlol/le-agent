...
from le_agent_coding.commands import create_default_command_registry
from le_agent_coding.prompt_templates import PromptTemplate
from le_agent_coding.skills import Skill
from le_agent_coding.tui.autocomplete import CompletionOption, build_completion_state
from le_agent_coding.tui.widgets import render_completion_suggestions
...
def test_theme_argument_completion_uses_theme_names() -> None:
    state = build_completion_state(
        ...
        skills=(),
        prompt_templates=(),
        theme_names=("le-agent-dark", "le-agent-light", "high-contrast"),
    )

    assert [item.display for item in state.items] == ["le-agent-dark", "le-agent-light"]
    assert state.selected is not None
    assert state.selected.apply("/theme le-agent-") == "/theme le-agent-dark"
