...
def test_tui_settings_path_uses_le_agent_home(tmp_path: Path) -> None:
    paths = LeAgentPaths(home=tmp_path / ".le-agent", agents_home=tmp_path / ".agents")

    assert tui_settings_path(paths) == tmp_path / ".le-agent" / "tui.json"
...
def test_save_tui_settings_writes_json(tmp_path: Path) -> None:
    ...
    path = save_tui_settings(TuiSettings(theme="le-agent-light"), paths)

    assert path == tmp_path / ".le-agent" / "tui.json"
    assert load_tui_settings(paths).theme == "le-agent-light"
...
def test_tui_settings_ignore_unknown_fields() -> None:
    settings = tui_settings_from_json(
        {
            "theme": "le-agent-light",
            "future_setting": {"enabled": True},
        }
    )

    assert settings.theme == "le-agent-light"
...
def test_tui_settings_accept_unknown_theme_and_fall_back_when_resolving() -> None:
    ...
    assert settings.resolved_theme == get_tui_theme("le-agent-dark")
...
def test_tui_settings_accept_light_theme() -> None:
    settings = tui_settings_from_json({"theme": "le-agent-light"})
...
def test_get_tui_theme_returns_builtin_theme() -> None:
    ...
    assert get_tui_theme("le-agent-light").prompt_border == "#2563eb"
