...
from le_agent_coding.tui.config import TuiSettings
...
def test_transcript_roles_are_shared_with_tui_state() -> None:
    from le_agent_coding.tui.state import ChatItemRole
    from le_agent_coding.tui.themes import TranscriptRole
...
def test_builtin_themes_are_loaded_from_packaged_json() -> None:
    assert BUILTIN_TUI_THEME_NAMES == ("le-agent-dark", "le-agent-light", "high-contrast")
    assert LE_AGENT_DARK_THEME.dark is True
    assert LE_AGENT_LIGHT_THEME.dark is False
    assert get_tui_theme("le-agent-dark").screen_background == "#000000"
    assert get_tui_theme("high-contrast").prompt_border == "#00ff66"
    assert get_tui_theme("le-agent-light").syntax_theme == "ansi_light"
...
def test_resolved_theme_falls_back_to_le_agent_dark_for_unknown_names() -> None:
    settings = TuiSettings(theme="missing-theme")

    assert settings.resolved_theme == LE_AGENT_DARK_THEME
...
def test_load_custom_themes_skips_builtin_shadowing_with_diagnostic(tmp_path: Path) -> None:
    themes_dir = tmp_path / "themes"
    _write_theme(themes_dir, "le-agent-dark.json", _theme_data(name="le-agent-dark"))

    themes, diagnostics = load_custom_tui_themes([themes_dir])

    assert themes == {}
    assert any("le-agent-dark" in diagnostic.message for diagnostic in diagnostics)
...
def test_tool_result_accents_derive_from_theme() -> None:
    ...
    from le_agent_coding.tui.state import ChatItem
    from le_agent_coding.tui.widgets import render_chat_item
...
def test_resource_paths_expose_theme_dirs(tmp_path: Path) -> None:
    paths = LeAgentResourcePaths(root=tmp_path / ".le-agent", cwd=tmp_path / "project")

    assert paths.themes_dirs == (
        tmp_path / ".le-agent" / "themes",
        tmp_path / "project" / ".le-agent" / "themes",
    )
