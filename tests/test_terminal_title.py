...
from le_agent_coding.tui.terminal_title import (
...
def test_build_terminal_title_uses_session_name_and_running_frame() -> None:
    assert build_terminal_title("build notes", running=False) == "le-agent | build notes"
    assert build_terminal_title("build notes", running=True, frame=1) == "⠙ le-agent | build notes"


def test_build_terminal_title_falls_back_for_unnamed_sessions() -> None:
    assert build_terminal_title(None, running=False) == "le-agent"
    assert build_terminal_title(" Untitled session ", running=True) == "⠋ le-agent"
...
def test_terminal_title_supported_requires_tty_and_allows_opt_out() -> None:
    ...
    assert not terminal_title_supported(
        environ={"TERM": "xterm-256color", "LE_AGENT_TERMINAL_TITLE": "0"},
        stream=TtyStringIO(),
    )
...
def test_terminal_title_controller_writes_running_idle_and_restore_titles() -> None:
    ...
    assert writes == [
        "\x1b]0;le-agent | build notes\x07",
        "\x1b]0;⠹ le-agent | build notes\x07",
        "\x1b]0;le-agent\x07",
    ]
