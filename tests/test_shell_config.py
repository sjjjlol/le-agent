...
def test_shell_settings_accepts_le_agent_style_shell_command_prefix() -> None:
...
def test_shell_settings_ignore_unknown_fields() -> None:
    settings = shell_settings_from_json(
        {
            "shellCommandPrefix": "source ~/.le-agent/aliases.bash",
            "future_shell_option": True,
        }
    )

    assert settings.shell_command_prefix == "source ~/.le-agent/aliases.bash"
